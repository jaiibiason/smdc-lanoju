...
const FeaturedProperties: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(propertiesRef, (snapshot) => {
      ...
      if (data) {
        ...
        const propertiesArray = Object.entries(data).map(([id, value]: [string, any]) => ({
          id,
          ...value,
        }));
  ...
  // Format price range from unit prices
  const formatPriceRange = (unitObj: Record<string, any> | undefined) => {
    if (!unitObj) return "Price on request";
    
    // Get all unit prices
    const prices = Object.values(unitObj)
      .filter((unit) => unit && typeof unit.price === 'number')
      .map((unit) => unit.price);
  ...
  const getPropertyStatus = (type: string | undefined) => {
    if (!type) return "Preselling";
    
    if (type.toLowerCase().includes("ready for occupancy")) {
      return "Ready for Occupancy";
    } else if (type.toLowerCase().includes("preselling")) {
      return "Preselling";
    }
    return type;
  };
              ...
              <Slider {...settings}>
                {properties.slice(0, 8).map((property) => (
                  <div 
                    ...
                    onClick={() => {
                      navigate(`/inner-prop/${property.id}`);
                      // The scroll will be handled by the useEffect in Inner Properties component
                    }}
                    style={{ cursor: "pointer" }}
                  >
                    <div className="property-image">
                      <img
                        src={property.images?.mainImage || import.meta.env.BASE_URL + "/assets/featured_properties/shore.png"}
                        alt={property.name}
                        className="property-image"
                      />
                      <p
                        className={`status ${
                          getPropertyStatus(property.type) === "Ready for Occupancy"
                            ? "ready"
                            : "preselling"
                        }`}
                      >
                        {getPropertyStatus(property.type)}
                      </p>
                    </div>
                    <div className="property-details">
                      ...
                      <p>
                        ...
                        {formatPriceRange(property.unit)}
                      </p>
                    </div>
                  </div>
                ))}
